    ...
    for variational in [True]:
        for nonlinear in [True]:
            for k in [1]:
                ...
                model.cross_validate(model=gibbs_sampler, train_data=train_set, test_data = test_set, path = 'results', train_epochs = 3001, lr = 0.00001, train_repeats = 20, batch_factor=1, ncrossval=1)
