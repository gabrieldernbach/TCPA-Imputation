    ...
    for variational in [True]:
        for nonlinear in [True]:
            for k in [1]:
                ...
                model.cross_validate(model=gibbs_sampler, train_data=train_set, test_data = test_set, path = 'results', train_epochs = 8001, lr = 0.0001, train_repeats = 15, batch_factor=1, ncrossval=1)
...
if calc_shapley:
    ...
    shapley.calc_all(device=device, steps=8000)
