...
conditional = False
triangle=True
load_epoch, load_variational, load_k, load_lin = 700, True, 1, 'nonlinear' #define model that shall be loaded for shapley
##################
plot = False
    ...
    for variational in [True]:
        for nonlinear in [True]:
            for k in [1]:
                ...
                model.cross_validate(model=gibbs_sampler, train_data=train_set, test_data = test_set, path = 'results', train_epochs = 701, lr = 0.0001, train_repeats = 15, batch_factor=1, ncrossval=1)
