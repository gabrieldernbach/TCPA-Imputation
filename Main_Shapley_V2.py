...
conditional = False
triangle=False
load_epoch, load_variational, load_k, load_lin = 3000, True, 1, 'nonlinear' #define model that shall be loaded for shapley
##################
    ...
    for variational in [True]:
        for nonlinear in [True]:
            for k in [1]:
                ...
                vae = model.VAE(input_dim=train_set.size(1), width=train_set.size(1)*64, sample_width=train_set.size(1)*128, depth=14, variational = variational, nonlinear = True, k = k)
                ...
                model.cross_validate(model=gibbs_sampler, train_data=train_set, test_data = test_set, path = 'results', train_epochs = 3001, lr = 0.0001, train_repeats = 10, batch_factor=1, ncrossval=1)
