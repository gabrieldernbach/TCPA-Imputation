...
for folder in ('results','results/figures', 'results/log', 'results/trained_model', 'results/adjacency','results/data', 'results/shapley', 'results/counter',
    ...
    for variational in [True]:
        for nonlinear in [True]:
            for k in [5]:
                ...
                vae = model.VAE(input_dim=train_set.size(1), width=train_set.size(1)*4, sample_width=train_set.size(1)*8, depth=3, variational = variational, nonlinear = True, k = k)
...
<<<<<<< Updated upstream
                model.cross_validate(model=gibbs_sampler, train_data=train_set, test_data = test_set, path = 'results', train_epochs = 501, lr = 0.0001, train_repeats = 1, batch_factor=5, ncrossval=1)
=======
                model.cross_validate(model=gibbs_sampler, train_data=train_set, test_data = test_set, path = 'results', train_epochs = 501, lr = 0.0001, train_repeats = 15, batch_factor=15, ncrossval=1)
>>>>>>> Stashed changes
