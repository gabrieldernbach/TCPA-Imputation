    ...
    for variational in [True]:
        for nonlinear in [True]:
            for k in [1]:
                ...
                vae = model.VAE(input_dim=train_set.size(1), width=train_set.size(1)*128, sample_width=train_set.size(1)*128, depth=10, variational = variational, nonlinear = True, k = k)
                ...
                model.cross_validate(model=gibbs_sampler, train_data=train_set, test_data = test_set, path = 'results', train_epochs = 2001, lr = 0.0001, train_repeats = 20, batch_factor=1, ncrossval=1)
