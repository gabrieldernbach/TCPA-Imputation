    ...
    for variational in [True]:
        for nonlinear in [True]:
            for k in [1]:
                ...
                model.cross_validate(model=gibbs_sampler, train_data=train_set, test_data = test_set, path = 'results', train_epochs = 1501, lr = 0.0001, train_repeats = 10, batch_factor=1, ncrossval=1)
...
if calc_hsic:
    ...
    shapley.calc_all(device=device, steps=150)
