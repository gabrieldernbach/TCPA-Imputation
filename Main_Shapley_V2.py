    ...
    for variational in [True]:
        for nonlinear in [True]:
            for k in [1]:
                ...
                vae = model.VAE(input_dim=train_set.size(1), width=train_set.size(1)*64, sample_width=train_set.size(1)*128, depth=10, variational = variational, nonlinear = True, k = k)
