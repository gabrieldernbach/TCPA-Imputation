...
conditional = False
triangle=False
load_epoch, load_variational, load_k, load_lin = 5, False, 1, 'nonlinear' #define model that shall be loaded for shapley
##################
    ...
    for variational in [False]:
        for nonlinear in [True]:
            for k in [1]:
                ...
                model.cross_validate(model=gibbs_sampler, train_data=train_set, test_data = test_set, path = 'results', train_epochs = 6, lr = 0.0001, train_repeats = 15, batch_factor=1, ncrossval=1)
