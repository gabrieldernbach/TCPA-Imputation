...
class Shapley:
    ...
    def calc_shapleypq(self, p, q, steps, device, probability):
        ...
        for t in range(1, 1+steps):
            ...
            for target, masked_data, Mask, in self.shapleyloader:
                ...
                if criterion(pred, target)<1000000000.0:
    ...
    def calc_all(self, device, steps, probabilities=[0.5]):
        for probability in probabilities:
                Parallel(n_jobs=5)(delayed(self.calc_shapleypq)(p, q, steps, device, probability) for p, q in itertools.product(range(self.nfeatures), range(self.nfeatures)) if p>q)
